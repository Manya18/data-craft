...
const InteractiveTable: React.FC = () => {
    ...
    const addColumn = async (columnName: string, columnType: string) => {
        if (newColumnName && !columns.includes(newColumnName)) {
            await fetch('http://localhost:4000/add-column', {
                method: 'POST',
                headers: {
    ...
    const addRow = async () => {
        ...
        setRows([...rows, newRow]);
        await fetch('http://localhost:4000/add-row', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    const handleDeleteRow = async (e: any) => {
        ...
        try {
            await fetch(`http://localhost:4000/data/${row.id}`, {
                method: 'DELETE',
            });
    ...
    const handleEditCell = async (rowId: number, columnKey: string, newValue: string) => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(`Ошибка обновления строки: ${response.statusText}`);
    ...
    const handleDeleteColumn = async (e: React.MouseEvent) => {
        ...
        if (!columnKey) {
            console.error('Ошибка: столбец для удаления не выбран.');
            return;
        }
        try {
            await fetch(`http://localhost:4000/columns/${columnKey}`, {
                method: 'DELETE',
            });
                            ...
                            >
                                {columns.map((column) => {
                                    ...
                                    return (
                                        <td
                                            key={column}
                                            onClick={() => handleClick(row.id, column.toLowerCase())}
                                        >
                                            {editingCell?.rowId === row.id && editingCell.columnKey === column.toLowerCase() ? (
                                                <input
                                                    ref={inputRef}
                                                    type="text"
                                                    ...
                                                    onChange={(e) => {
                                                        const newValue = e.target.value;
                                                        setRows(rows.map(r => (r.id === row.id ? { ...r, [editingCell.columnKey]: newValue } : r)));
                                                    }}
                                                    onKeyDown={(e) => handleKeyDown(e, row.id, column.toLowerCase())}
